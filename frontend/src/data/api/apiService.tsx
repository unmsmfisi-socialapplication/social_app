...
export async function doPost<T>(url: string, params: Record<string, any> = {}): Promise<AxiosResponse<T>> {
    const idToken = localStorage.getItem('token') || ''
    try {
        const response = await axios({
            url,
...
export async function doDelete<T>(url: string, params: Record<string, any> = {}): Promise<AxiosResponse<T>> {
    const idToken = localStorage.getItem('token') || ''
    try {
        const response = await axios({
            url,
